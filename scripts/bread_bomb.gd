...
@export var bread_bomb_explosions: PackedScene
...
func _process(delta):
	bread_sprite.rotation_degrees += rotation_speed * delta * velocity.normalized().x
...
func _physics_process(delta):
	...
	if progress >= 1.0:
		var new_explosion = bread_bomb_explosions.instantiate()
