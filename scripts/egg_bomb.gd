...
var is_straight_line: bool = false
...
func _process(delta):
	egg_sprite.rotation_degrees += rotation_speed * delta * velocity.normalized().x
...
func _physics_process(delta):
	...
	if is_straight_line:
		...
		global_position = start_position + (direction * base_speed * current_time)
	else:
